...
> [!info] Operadores de comparación
> **Numérica:** `-eq` igual · `-ne` distinto · `-lt` menor · `-le` menor o igual · `-gt` mayor · `-ge` mayor o igual.
> **Cadenas:** `==` igual · `!=` distinto · `-z` cadena vacía · `-n` cadena no vacía.
> **Archivos:** `-f` archivo regular · `-d` directorio · `-e` existe · `-r/-w/-x` permisos · `-s` no vacío.
...
ALERTA_EMAIL="admin@ejemplo.local"
...
> [!tip] Plantilla mínima robusta
> Empieza todo script con `#!/bin/bash` y `set -euo pipefail`. Es la base para que los errores no pasen desapercibidos.

---
🔗 Relacionado: [[09 - Backups y recuperación]] · [[06 - Servicios con systemd]] · [[11 - Virtualización y contenedores]]
