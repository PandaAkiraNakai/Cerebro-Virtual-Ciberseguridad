...
tags:
  ...
  - recuperacion
aliases:
  - Backups y recuperación en Linux
  - rsync, tar y borg
---
...
> [!info] Opciones útiles de rsync
> `-a` archive (permisos, dueño, fechas, symlinks) · `-v` verbose · `-z` compresión · `-n` dry run · `-P` progreso y reanudar · `--delete` espejo · `--bwlimit=1000` limitar a 1MB/s.
...
> [!warning] Cuidado con dd
> `dd` no pregunta nada. Un `of=` equivocado puede destruir el disco erróneo. Verifica dos veces antes de ejecutar.
...
> [!tip] Regla de oro
> Un backup no verificado no vale nada. Restaura periódicamente en un entorno de prueba.
...
---
🔗 Relacionado: [[08 - Seguridad del servidor]] · [[04 - Almacenamiento y filesystems]] · [[10 - Scripting y automatización]]
