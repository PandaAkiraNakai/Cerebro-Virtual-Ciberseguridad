...
tags:
  ...
  - rendimiento
aliases:
  - Procesos y rendimiento en Linux
...
btop                        # Monitor moderno y visual (apt install btop)
...
nice -n -5 comando          # Prioridad más alta (requiere root para negativos)
...
> [!info] Cómo leer el load average
> Un load average igual al número de CPUs equivale al 100% de uso. Si tienes 4 CPUs y el load es 4.0, el servidor está al límite. Si es 8.0, está saturado.
...
cat /proc/PID/oom_score                    # Puntuación OOM (mayor = más riesgo)
...
> `kill -9` no permite que el proceso limpie sus recursos (archivos temporales, locks, conexiones). Úsalo solo cuando SIGTERM no funcione.

---
🔗 Relacionado: [[02 - Red y conectividad]] · [[04 - Almacenamiento y filesystems]]
